...
// 234 ntp servers
// The beta_NTP_servers variable is a hard-coded version of the identified NTP servers to draw a CDF graph of the NTP servers registered on the beta site (as of July 22, 22nd, 22nd, 22nd).
// In order to reproduce the experiment, the list of NTP servers must be updated based on the time of the experiment. 
...
// The beta_NTP_servers array is being used to store server names.
  ...
  fs.appendFile({Relative path to save experiment 2 results as txt file.}, beta_NTP_servers[refresh_cnt] + ',' +extractedStr, err => {
    if (err) {
      console.error(err)
      return;
    }
